...
class Grid:
    ...
    def checkforpossibleimprovewinning(self,piece1,piece2, usertype):
        #####################################################################################
        #       Developer:      Jacob Croes
        #       Definition:     Checks to see if the two pieces in a row belong to the same player
        #####################################################################################
    ...
    def displaywinningmessage(self,win):
        ...
        if (returnvalue == 'restart'):
            ...
            if (returnvalue == 'OnePlayer'):
                returnvalue = z.screenmaking("Choose difficulty" , "", BLUE ,RED ,BLACK , "Easy", "Hard", 'difficult')
                if (returnvalue == 'Easy'):
                    self.difficulty = 1
                else:
                    self.difficulty = 2
